...
const BookMarkFav = ({bookMenuOpen,onClose,handleRemoveFavorites,favoritos,isDarkMode}) => {

    
    return(
    <div
    className={` fixed right-0 top-0 h-screen w-80 ${isDarkMode ? 'dark:bg-[#292b25] dark:text-[#e7edea]' : 'bg-slate-100 ' }   transition-transform transform ${
  ...
  >
    ...
    <section className="p-2 mt-5">
      <button onClick={onClose} className={`${isDarkMode ? 'dark:bg-[#363931] dark:hover:bg-[#292b25] dark:text-[#e7edea]' : 'bg-[#e7e7e9] hover:bg-[#dbdbdb] text-[#0d0c22]'}    py-2 px-4 rounded-lg duration-300  text-lg`}>X</button>
      <div className="flex flex-col gap-2 text-center">
          <h2 className="my-2 text-2xl font-medium underline underline-offset-4">Libros Agregados</h2>

          {favoritos.map((book) => (
            <div key={book.id} className={`${isDarkMode ? 'dark:hover:bg-[#363931]' : 'hover:bg-gray-300'} relative flex flex-row items-center p-2 transition duration-200 rounded-md group  `}>
              ...
              <p className="mb-2 text-gray-600 dark:text-[#a5a3a3]">{book.author.name}</p>
